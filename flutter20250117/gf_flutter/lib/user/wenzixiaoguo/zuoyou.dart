...
class Zuoshangyouxia_text extends StatefulWidget {
  const Zuoshangyouxia_text({Key? key}) : super(key: key);

  @override
  State<Zuoshangyouxia_text> createState() => _Zuoshangyouxia_textState();
}

class _Zuoshangyouxia_textState extends State<Zuoshangyouxia_text> with SingleTickerProviderStateMixin {
  ...
  void initState() {
    ...
    // 圆环大小动画
    _radiusAnimation = Tween<double>(begin: 0.3, end: 0.5).animate(
      CurvedAnimation(
        parent: _controller,
            ...
            return Container(
              ...
              child: CustomPaint(
                painter: CircleTextPainter(
                  ...
                  yPos: _yAnimation.value,
                  radiusRatio: _radiusAnimation.value,
                ),
...
class CircleTextPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    for (int i = 0; i < text.length; i++) {
      ...
      final bool isTouched = distance >= maxRadius - circleWidth/2 && distance <= maxRadius + circleWidth/2;

      textSpans.add(TextSpan(
        text: text[i],
        style: TextStyle(
          fontSize: isTouched ? 28 : 24,
          fontWeight: FontWeight.bold,
          color: isTouched ? highlightColor : textColor,
        ),
      ));
    }
