...
class shangchuanshezhi extends StatefulWidget {
  const shangchuanshezhi({Key? key}) : super(key: key);

  @override
  _shangchuanshezhiState createState() => _shangchuanshezhiState();
}
...
class _shangchuanshezhiState extends State<shangchuanshezhi> {
...
//                                       Map<String, dynamic> parameter1 = {
                                       
//                                         "Add_authorrity": Controller_menuurl?.text,
//                                         "Rolename":"user",
//                                         "token": token,
                                       
//                                       };
// var response1=await YXHttp().http_post(   map0api["增加或减少角色权限"]!, parameter1);
                                                        ...
                                                        child:
                                                          ...
                                                          header: Column(
                                                            ...
                                                            children: <Widget>[
                                                              Text(
                                                               '''
1新增加菜单或修改菜单url路由要修改后台casbin_rule表,把修改后的菜单路由增加到admin权限下.
2修改前台源码的model1.dart文件的menu_admin_class变量,增加的参数格式为:菜单名:菜单页面类.  
3在数据库casbin_rule表,把修改后的菜单路由增加到admin权限下.
                                                                ''',
                                                                style:
                                                                    TextStyle(
                                                                  fontSize: 12,
                                                                    color:
                                                                      const Color.fromARGB(255, 255, 73, 7)
                                                                ),
                                                              ),
                                                            ],
                                                          ),
            ...
            Container(
                ...
                child: Row(children: [
                  ...
                  data[index]["fuhao"] == "+"
                      ...
                      : Row(children: [
                          SizedBox(width: 10 * 2),//二级菜单留空
                          //  SizedBox(width: 10 * 2,child: Text("----")),//二级菜单留空
              ...
              children: [
                ...
                ElevatedButton(
                  onPressed: () async {
                    ...
                    for (int i = 0; i < caidan_admin_1_2.length; i++) {
                      bool iscontain =
                          caidan_admin_1_2[i].contains(data[index]["MenuName"]);
                      print("iscontain");
                      print(iscontain);
                      if (iscontain) {
                        index1 = i;
                    ...
                    if ((caidan_admin_1_2[index1].length > 1) &
                        (data[index]["ParentId"] == "0")) {
                      //判断caian_admin12的索引数据长度,如果>1,说明含有二级菜单.caidan_admin_1_2=[[网站信息, 站点域名, 站点名称], [系统设置, 功能设置, 文件存储], [用户管理, 用户信息], [内容功能, 文章发布, 文章管理, 招聘发布, 招聘管理], [其他, seo], [用户中心, 个人中心, 资料修改], [测试一级菜单]]
                      print("含有二级菜单,无法删除!");
                      Get.Get.snackbar("结果", "含有二级菜单,无法删除!",
                          snackPosition: SnackPosition.BOTTOM,
                          duration: Duration(seconds: 1));
                      return;
                    } else {
                      caidan_admin_1_2[index1].remove(
                          data[index]["MenuName"]); //在一级二级菜单名称的列表中移除删除的菜单项名称.
                      //  print('caidan_admin_1_2:');
                      //         print(caidan_admin_1_2);
                      print("继续执行!");
                      delete_data(index, data[index]["Id"]);
                      data.removeAt(index); //数据源移除该索引数据.
