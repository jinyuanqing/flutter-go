...
class wenzhang_bianji extends StatefulWidget {
  final title;
  final nei_rong;
  final id;
  final zuo_zhe;
  ...
  const wenzhang_bianji(
      {Key? key,
      @required this.id,
      @required this.title,
      @required this.nei_rong,
      @required this.zuo_zhe,
      @required this.zhaiyao,
      @required this.suoluetu,
      @required this.fenlei_id})
      : super(key: key); //注意这是需要参数的类的定义,继承自StatelessWidget

  @override
  State<StatefulWidget> createState() => new wenzhang_bianji1();
}
...
class wenzhang_bianji1 extends State<wenzhang_bianji>
  ...
  void initState() {
    ...
    thumbnail = widget.suoluetu;
    fenlei_id = widget.fenlei_id;
    title_control?.value = TextEditingValue(text: widget.title); //标题
    author_control?.value = TextEditingValue(text: widget.zuo_zhe); //作者
    zhaiyao_control?.value = TextEditingValue(text: widget.zhaiyao); //摘要
    wenzhang_id = widget.id;
    _loadDocument();

    get_fenlei();

    //  fenlei[0]=res[i]["fenlei_name"];
...
// // 或者通过传递一个 `options`来创建dio实例
      ...
      if (response.statusCode == 200) {
        if (response.data["code"] == "0") {
          ...
          for (var i = 0; i < res.length; i++) {
            ...
            if (res[i]["id"] == widget.fenlei_id) {
              // dropdownValue = res[i]["fenlei_name"];
              fenlei[0] = "当前" + res[i]["fenlei_name"];
            }
          }
          setState(() {
            _streamController1.sink.add(fenlei.first);
  ...
  Future<void> _loadDocument() async {
    var s = widget.nei_rong.replaceAll('\n', '\\n');
